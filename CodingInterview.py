...
# table = []
# for i in range(n):
#     table += [int(v) for v in input().split()]
from collections import Counter
import operator
n = 3
m = 3
table = [[1,1,1],[1,5,1],[1,1,1]]
A = []
B = []
for i in range(n):
    for j in range(m):
...
A_tp = Counter(A)
B_tp = Counter(B)
A_tp2 = sorted(A_tp.items(),key=operator.itemgetter(1),reverse=True)
B_tp2 = sorted(B_tp.items(),key=operator.itemgetter(1),reverse=True)
out = 0
if len(A_tp2)==1 and len(B_tp2)==1:
    if A_tp2[0][0] == B_tp2[0][0]:
        out = min(len(A),len(B))
    else:
        out = 0
elif len(A_tp2)>1 or len(B_tp2)>1:
    # if A_tp2[0][0] != B_tp2[0][0]:
    out = min(len(A) - A_tp2[0][1], len(B) - B_tp2[0][1])
    # else:
    #     out = 0
...
# 4. Median of Two Sorted Arrays
    def p_004_findMedianSortedArrays(self, nums1: list, nums2: list) -> float:
...
#35. Search Insert Position
    def p_035_searchInsert(self, nums: list, target: int) -> int:
...
#23. Merge k Sorted Lists
    def p_023_mergeKLists(self, lists: list) -> ListNode:
...
#                 mylists[0] = mylists[0].next
    ...
    #*******************************回溯法***********************
    #78. Subsets
    def p_078_subsets(self, nums: list) -> list:
        # def backtrack(ans:list,tmp:list,nums:list,start:int):
        #     ans.append(tmp.copy())
    ...
    # 90. Subsets2
    def p_090_subsetsWithDup(self, nums: list) -> list:
        if 0:
            ans = [[]]
            lgth = 0
            nums.sort()
            for i,val in enumerate(nums):
                if i>0 and val==nums[i-1]:
                    ans+= [ ans[m]+[val] for m in range(-lgth,0) ]
                else:
                    tmp = [item+[val] for item in ans]
                    lgth = len(tmp)
                    ans+= tmp
            return ans
        else:

            def helper(cur, A, index, result):
                result.append([x for x in cur])
                if index >= len(A):
                    return
    ...
    class p_155_MinStack:
        ...
        def __init__(self):
            ...
            self.ndToRecal = False

        def push(self, x: int) -> None:
            self.myStack.append(x)
            self.myMin = min(self.myMin, x)

        def pop(self) -> None:
            tmp = self.myStack.pop()
            if tmp < (self.myMin + 0.1) and tmp > (self.myMin - 0.1):
                self.ndToRecal = True

        def top(self) -> int:
            return (self.myStack[-1] if len(self.myStack) > 0 else None)
        ...
        def getMin(self) -> int:
            if self.ndToRecal:
                ...
                for v in self.myStack:
                    self.myMin = min(self.myMin, v)
            return self.myMin


    #104. Maximum Depth of Binary Tree
    def p_104_maxDepth(self, root: TreeNode) -> int:
        if root == None:
            return 0
    ...
    #55. Jump Game
    def p_055_canJump(self, nums: list) -> bool:
        if 0:
            # 这种思路还存在缺陷
            have0 = False
            Inprocess = False
    ...
    def p_029_divide(self, dividend: int, divisor: int) -> int:
        sign = 1
    ...
    def p_017_letterCombinations(self, digits: str) -> list:
        tab = ["","","abc","def","ghi","jkl","mno","pqrs","tuv","wxyz"]
        ans = [""]
        for v in digits:
            tmp = int(v)
            if tmp<2:continue
            lenth_before = len(ans)
            ans *= len(tab[tmp])
            for i in range(len(ans)):
                ans[i] = ans[i] + tab[tmp][i//lenth_before]
        return ans

    #5. Longest Palindromic Substring
    def p_005_longestPalindrome(self, s: str) -> str:
        if len(s)==1: return s
        elif len(s)==0: return ""
    ...
    # 139. Word Break
    def p_139_wordBreak(self, s:str, wordDict:list)->bool: #wordBreak(self, s: str, wordDict: list[str]) -> bool:
    ...
    def p_014_longestCommonPrefix(self, strs: list) -> str:#longestCommonPrefix(self, strs: List[str]) -> str:
        if len(strs) <= 0:
            return ""
        elif len(strs)==1:
            return len(strs[0])
    ...
    def p_020_isValid(self, s: str) -> bool:
        N = len(s)
        if N == 0:
            return True
        stack = []
    ...
    def p_021_mergeTwoLists(self, l1: ListNode, l2: ListNode) -> ListNode:

        t1 = l1
        t2 = l2
        out = ListNode(0x7FFFFFFF)
        out_t = out
        while t1 != None or t2 != None:
            if t1 == None:
                out_t.next = ListNode(t2.val)
                t2 = t2.next
            elif t2 == None:
                out_t.next = ListNode(t1.val)
                t1 = t1.next
            else:
                if t1.val < t2.val:
                    out_t.next = ListNode(t1.val)
    ...
    def p_217_containsDuplicate(self, nums: list) -> bool:
        length = len(nums)
        tab = dict()
    ...
    def p_206_reverseList(self, head: ListNode) -> ListNode:
        p = head
        pLast = None
        ptmp = None
        while p!=None:
...
#215. Kth Largest Element in an Array
    def p_215___partSort(self, nums:list, l:int, r:int):
        #length = r - l + 1
        g = l  # random.randint(0,length-1)
        i = l + 1
        j = r
        while j >= i:
            if nums[i] < nums[g] and nums[j] > nums[g]:
                ...
                if nums[j] <= nums[g]:
                    j = j - 1
        if i > l + 1:
            nums[g] = nums[g] ^ nums[i-1]
            nums[i-1] = nums[i-1] ^ nums[g]
            nums[g] = nums[g] ^ nums[i-1]
        return i-1

    ##215. Kth Largest Element in an Array
    def p_215_findKthLargest(self, nums: list, k: int) -> int:
        ...
        return nums[k_]
#204. Count Primes
    def p_204_countPrimes(self, n: int) -> int:
        tab = [False]*n
        cnt = 0
        for i in range(2,n):
            if tab[i] == False:
    ...
    def firstMissingPositive(self, nums: list) -> int:
        ...
        length = len(nums)
        for i in range(0, length):
            while nums[i] > 0 and nums[i] <= length and nums[nums[i] - 1] != nums[i]:#nums[i] != i + 1 and nums[i] != nums[nums[i] - 1]:
                tmp = nums[nums[i] - 1]
                nums[nums[i] - 1] = nums[i]
                nums[i] = tmp
                #nums[i], nums[nums[i] - 1] = nums[nums[i] - 1], nums[i] #这种方法在这里不起作用
        for i in range(0, length):
            if nums[i] != i + 1:
                return i + 1
        return length + 1
    ...
    def strStr(self, haystack: str, needle: str) -> int:
        ...
        while True:#i
            ...
            i = i + 1
#15. 3Sum
    def p_015_threeSum(self, nums: list) -> list:#List[List[int]]:
        nums.sort()
        lth = len(nums)
        res = []
        for i in range(0,lth):
            if i==0 or (i > 0 and nums[i] != nums[i - 1]): #注意与后面比较，和与前面比较的区别
                for j in range(i+1,lth):
                    if j==i+1 or (j >i+1 and nums[j] != nums[j - 1]): #注意与后面比较，和与前面比较的区别
                        #if j + 1 < lth and nums[j] == nums[j + 1]: continue
                        tgt = -(nums[i] + nums[j])
                        l,r = j+1,lth-1
                        if l<lth:
                            if tgt == nums[r]:
                                res.append([nums[i], nums[j], nums[r]])
                            elif tgt == nums[l]:res.append([nums[i], nums[j], nums[l]])
                            elif tgt > nums[l] and tgt < nums[r]:
                                while l <= r:
                                    mid = l + int((r - l) / 2)
                        ...
                        #这种方法在数据量大的时候，会超时，所以现在我们来采用二分法的方式
                        # k = j + 1
                        # while k < lth and tgt >= nums[k]:
                        #     if tgt == nums[k]:
...
#53. Maximum Subarray
    def p_053_maxSubArray(self, nums: list) -> int:
        mthd = 0
        if mthd:
            minsum = 0
            out = float("-inf")
...
#121. Best Time to Buy and Sell Stock
    def p_121_maxProfit(self, prices: list) -> int:
        mthd = 0 #两种方法差不多
        if mthd:
            lth = len(prices)
            if lth<=1:return 0
            ...
            for i in range(1,lth):
                curPt = prices[i] - minBy
                #dp[i+1] = prices[i] - minBy
                minBy = min(minBy,prices[i])
                maxPt = max(maxPt,curPt)
...
#198. House Robber
    def p_198_rob(self, nums: list) -> int:
        ft = 0
        bk = 0
        for val in nums:
            tmp = ft
...
#746. Min Cost Climbing Stairs
    ...
    def p_746_minCostClimbingStairs(self, cost: list) -> int:
        mthd = 0
        if mthd:
            lth = len(cost)
            #ft = 0
            ...
            for i in range(2,lth):
                dp[i] = cost[i] + min(dp[i-1],dp[i-2])#min(cost[i] + dp[i-1],cost[i-1] + dp[i-2])
                #min(cost[i]+ft,cost[i-1]+ft)
            return min(dp[lth-1],dp[lth-2])
        else:
...
#120. Triangle
    def p_120_minimumTotal(self, triangle: list) -> int:
        out = []
        length = len(triangle)
        out = [triangle[0]]
        for i in range(1, length):
            ...
            for j in range(0, i + 1):
                ...
                if l < 0: l += 1
                if r == i: r -= 1
                tmp.append(triangle[i][j] + min(out[i - 1][l], out[i - 1][r]))
            out.append(tmp)
        return min(out[length - 1])
    ...
    '''
#621. Task Scheduler
    def p_621_leastInterval(self, tasks: list, n: int) -> int:

        tab = dict()
        cnt = 0
        for val in tasks:
            tab[val] = tab[val] + 1 if val in tab.keys() else 1
            cnt = max(cnt, tab[val])
        out = (cnt - 1) * (n + 1)
...
#48. Rotate Image
    def p_048_rotate(self, matrix: list) -> None:
        """
...
#139. Word Break
#动态规划
print( Solution().p_139_wordBreak("leetcode",["leet", "code"]) )
print( Solution().p_139_wordBreak("applepenapple",["apple", "pen"]) )
print( Solution().p_139_wordBreak("catsandog",["cats", "dog", "sand", "and", "cat"]) )

# 14. Longest Common Prefix
print( Solution().p_014_longestCommonPrefix(["flower","flow","flight"]) )
print( Solution().p_014_longestCommonPrefix(["a"]) )

# 20. Valid Parentheses
print( Solution().p_020_isValid("()") )
...
ll2.next = ListNode(3)
ll2.next = ListNode(4)
print( Solution().p_021_mergeTwoLists(ll1,ll2) )

#217. Contains Duplicate
print( Solution().p_217_containsDuplicate([1,2,3,4,1]) )

#206. Reverse Linked List
print( Solution().p_206_reverseList(ll1) )

##215. Kth Largest Element in an Array
# print( Solution().p_215_findKthLargest([3,2,1,5,6,4],2) )
# print( Solution().p_215_findKthLargest([2,1],1) )
# print( Solution().p_215_findKthLargest([2,1],2) )
# print( Solution().p_215_findKthLargest([5,2,4,1,3,6,0],4) )

#204. Count Primes
print( Solution().p_204_countPrimes(10))
...
#78. Subsets
print(Solution().p_078_subsets([1,2,3]))

#90. Subsets2
print(Solution().p_090_subsetsWithDup([4,4,4,1,4]))
...
print(Solution().p_023_mergeKLists(data_in))

#35. Search Insert Position
print(Solution().p_035_searchInsert([1,3,5,6],0))

# 4. Median of Two Sorted Arrays
print(Solution().p_004_findMedianSortedArrays([1,3,5,7,9],[2,4,6,8,10,12]))
