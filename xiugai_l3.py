...
import pandas as pd
import numpy as np
jieguo = pd.read_csv('dfOut_3nd_l3.csv',encoding = 'gbk')
haveRealResult = False
filename = '321data_test.csv'
if haveRealResult == True:
    filename = '321data.csv'
...
if loading == False:
    ...
    jieguo1.to_csv('dfOut_3nd_l3_xiugai3.csv',encoding='gbk',index=True)
else:
    jieguo1 = pd.read_csv('dfOut_3nd_l3_xiugai3.csv', encoding='gbk')
