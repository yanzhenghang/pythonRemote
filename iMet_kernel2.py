...
workingDir = 'Cp2_Epoch2_Working'
import os
os.environ["CUDA_VISIBLE_DEVICES"]="1"
srcPercent = 0.05
SIZE_test = 0.12
myDir = "/home/yanzhenghang/kaggle/kernel/1.190413 pretrained Resnet50 + focal loss/"#'1.190413 pretrained Resnet50 + focal loss'"
if not os.path.isdir(myDir+'../'+workingDir):
    os.mkdir(myDir+'../'+workingDir)
...
# Load dataset info
path_to_train = myDir+'../input/imet-2019-fgvc6/train/'
data = pd.read_csv(myDir+'../input/imet-2019-fgvc6/train.csv')
...
def create_model(input_shape, n_out):
    ...
    base_model.load_weights(myDir+'../input/resnet50/resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5')
...
epochs = 25
batch_size = 32
checkpoint = ModelCheckpoint(myDir+'../'+workingDir+'/Resnet50_focal.h5', monitor='val_loss', verbose=1,
...
csv_logger = CSVLogger(filename=myDir+'../'+workingDir+'/training_log.csv',
                       separator=',',
                       append=True)
...
indexes = np.arange(int(train_dataset_info.shape[0]*srcPercent))
train_indexes, valid_indexes = train_test_split(indexes, test_size=SIZE_test, random_state=7)
...
print(os.listdir(myDir+'../'+workingDir+'/'))
...
epochs = 20
batch_size = 32
checkpoint = ModelCheckpoint(myDir+'../working/Resnet50_focal.h5', monitor='val_loss', verbose=1,
                             save_best_only=True, mode='min', save_weights_only=True)
...
csv_logger = CSVLogger(filename=myDir+'../working/training_log.csv',
                       separator=',',
                       append=True)

# split data into train, valid
indexes = np.arange(train_dataset_info.shape[0])
train_indexes, valid_indexes = train_test_split(indexes, test_size=0.2, random_state=7)
...
print(os.listdir(myDir+'../working/'))

submit = pd.read_csv(myDir+'../input/imet-2019-fgvc6/sample_submission.csv')
model.load_weights(myDir+'../working/Resnet50_focal.h5')
predicted = []
...
for i, name in tqdm(enumerate(submit['id'])):
    path = os.path.join(myDir+'../input/imet-2019-fgvc6/test/', name)
...
submit.to_csv(myDir+'submission.csv', index=False)
